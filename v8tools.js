...
// precompiled version
var debug = require('debug')('APM-Monitor.v8tools');
var module_name = "v8tools";
...
for (key in version_map) {
    if (new RegExp(key).test(process.version)) {
...
try {
    ...
    module.exports = require('bindings')('v8tools');
}
catch (err) {
    //debug('binding load error:' + err);
    try {
        debug('try to load library from ' + modpath);
